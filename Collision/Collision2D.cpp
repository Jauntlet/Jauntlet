...
bool Collision2D::calcCollision(CircleCollider2D* parent, BoxCollider2D* other) {
	...
	if (t.x < other->position.x - other->GetWidth() * 0.5f) {
		t.x = other->position.x - other->GetWidth() * 0.5f;
	} else if (t.x > other->position.x + other->GetWidth() * 0.5f) {
		t.x = other->position.x + other->GetWidth() * 0.5f;
	}

	if (t.y < other->position.y - other->GetHeight() * 0.5f) {
		t.y = other->position.y - other->GetHeight() * 0.5f;
	} else if (t.y > other->position.y + other->GetHeight() * 0.5f) {
		t.y = other->position.y + other->GetHeight() * 0.5f;
	}
	...
	if (tDist <= parent->GetRadius()) {
		...
		if (relCenter.x == relCenter.y) {
			newNormal = glm::normalize(relCenter);
		} else if (glm::abs(relCenter.x) > glm::abs(relCenter.y)) {
			newNormal = glm::normalize(glm::vec2(relCenter.x, 0));
		} else {
			newNormal = glm::normalize(glm::vec2(0, relCenter.y));
		}
...
bool Collision2D::calcCollision(BoxCollider2D* parent, CircleCollider2D* other) {
	...
	if (t.x < parent->position.x - parent->GetWidth() * 0.5f) {
		t.x = parent->position.x - parent->GetWidth() * 0.5f;
	} else if (t.x > parent->position.x + parent->GetWidth() * 0.5f) {
		t.x = parent->position.x + parent->GetWidth() * 0.5f;
	}
	
	if (t.y < parent->position.y - parent->GetHeight() * 0.5f) {
		t.y = parent->position.y - parent->GetHeight() * 0.5f;
	} else if (t.y > parent->position.y + parent->GetHeight() * 0.5f) {
		t.y = parent->position.y + parent->GetHeight() * 0.5f;
	}
...
bool Collision2D::calcCollision(BoxCollider2D* parent, BoxCollider2D* other) {
		...
		parent->position.y + parent->GetHeight() > other->position.y)
	{
		...
		if (relCenter.x == relCenter.y) {
			...
			newNormal = relCenter.x > 0 ? glm::vec2(-1, 0) : glm::vec2(1, 0);
		} else {
			newNormal = relCenter.y > 0 ? glm::vec2(0, -1) : glm::vec2(0, 1);
		}
...
bool Collision2D::calcCollision(CircleCollider2D* parent, glm::vec2 other) {
	if (JMath::Distance(parent->position + glm::vec2(parent->GetRadius()), other) <= parent->GetRadius()) {
		return true;
	}
...
bool Collision2D::calcCollision(BoxCollider2D* parent, glm::vec2 other) {
	if (other.x >= parent->position.x &&
		...
		other.y <= parent->position.y + parent->GetHeight())
	{
		return true;
	}
